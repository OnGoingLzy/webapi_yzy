...
CUuTOweK6xfddX2xK9Z+y2AR7I0PK/FRboaa0+yP7BClla0woPgWwimh7xUccfCg3XIYF6gFHL5Zsolj/ZeXhw==
        ...
        public string GenerateSignature(string timestamp,string transSerial, string transInput,string yyjgbm)
        {
            try
            {
        ...
        public string RSAEncrypt( string m_strEncryptString)
        {
            ...
            byte[] bytes_old = Encoding.GetEncoding("utf-8").GetBytes(m_strEncryptString);

            byte[] bytes = Compress(bytes_old);
        ...
        public string generateYinhaiQueryOrderParam(string main_order_id)
        {
            ...
            //生成签名
            string sign_data = GenerateSignature(timestamp, sender_trns_sn, encryptReqData, (string)dbResObj["medins_code"]);
            yinhaiInputObject.sign_data = sign_data;
            string url = "http://zhuguangcao.mynatapp.cc/dsmp-api/api/public/app/T9152";
        ...
        public string generateYinhaiQueryRefundOrderParam(string main_order_id)
        {
            ...
            string sign_data = GenerateSignature(timestamp, sender_trns_sn, encryptReqData, (string)dbResObj["medins_code"]);
            yinhaiInputObject.sign_data = sign_data;
            string url = "http://zhuguangcao.mynatapp.cc/dsmp-api/api/public/app/T9155";
            HttpMethod method = HttpMethod.Post;
            //请求报文转为json
            string jsonData = JsonConvert.SerializeObject(yinhaiInputObject);
        ...
        public async Task<string> sendSyncAcssTokenRequestAsync()
        {
            ...
            yinhaiInputObject.sign_data = sign_data;
            string url = "http://zhuguangcao.mynatapp.cc/dsmp-api/api/public/app/T8201";
